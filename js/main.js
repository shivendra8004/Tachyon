...
function onResize() {
  ...
  smokeCanvas.style.setProperty(
    "height",
    `${s3Ground.getBoundingClientRect().top - svgRect.top}px`
  );
...
function step(now) {
  ...
  if (sy < sceneRects[0].height) {
    s1MarsSurface.style.setProperty(
      "transform",
      `translateY(${(((-sy * 120) / (120 + 240)) * 640) / s1MarsWidth}px)`
    );
    s1MarsShadow.style.setProperty(
      "transform",
      `translateY(${(((-sy * 120) / (120 + 480)) * 640) / s1MarsWidth}px)`
    );
    s1Tip.style.setProperty(
      "opacity",
      sy > sceneRects[0].height * 0.25 ? 0 : 1
    );
  }

  // Scene 3
  const s3 =
    (sy + containerHeight - sceneRects[2].top) /
    (sceneRects[2].height + containerHeight);
  if (s3 >= 0 && s3 <= 1) {
    ...
    const shakeIntensity =
      shakeIntensity0 - Math.min(Math.max((s3 - 0.6) / 0.2, 0), 1);

    //                             Start Duration Scale down so it wont go above 1
    let rocketTranslation = (s3 - 0.32) / 0.5 / 1.36;
    rocketTranslation =
      cubicBezier(0.25, 0, 0.5, 0.1, rocketTranslation) *
      (sceneRects[2].height / 2) *
      1.36;
    ...
    s3Tower1.style.setProperty(
      "transform",
      `rotate(${shake1[2] * shakeIntensity}deg)`
    );
    s3Tower2.style.setProperty(
      "transform",
      `rotate(${shake2[2] * shakeIntensity}deg)`
    );
    ...
    let glow =
      Math.min(Math.max((s3 - 0.4) / 0.1, 0), 1) -
      Math.min(Math.max((s3 - 0.6) / 0.2, 0), 1);
    s3Glow.style.setProperty("opacity", glow);
  }

  // Scene 5
  const s5 =
    (sy + containerHeight - sceneRects[4].top) /
    (sceneRects[4].height + containerHeight);
  if (s5 >= 0 && s5 <= 1) {
  ...
  s6Credit.classList.toggle(
    "Credit-active",
    sy > sceneRects[5].top - sceneRects[5].height * 0.01
  );
...
function step2() {
  ...
  ctx.translate(
    -(1000 - smokeCanvasWidth / smokeScale) / 2,
    -(1000 - svgHeight / smokeScale) / 2
  );
